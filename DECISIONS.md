...
Code Decision #7: Lens 문장 구조 단순화

Context: 실제 이메일과 웹뷰어로 결과물을 받아보고 나서, 콘텐츠 자체는 insightful한데 문장이 길고 복잡해서 가독성이 떨어진다고 판단.
Decision: 톤·깊이·콘텐츠 본질은 건드리지 않고 문장 구조만 수정. 한 문장 한 아이디어, 최대 20단어 규칙을 4개 Lens 프롬프트 전체에 추가.
Reasoning: 내용 자체가 문제가 아니라 표현 방식만 문제였기 때문. 분석의 깊이를 희생하지 않고 스캔 가능한 형태로 다듬는 게 가능하다고 판단.
Claude's role: Tech Lens 하나만 먼저 Before/After 샘플 출력 비교 → OK 받은 뒤 4개 전체 적용.
